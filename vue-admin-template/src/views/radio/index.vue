  ...
  <div id="content">
    ...
    <div class="conBottom" :class="{'active':isActive}">
      <div class="conLeft">
        ...
        <ul>
          <li v-for="item in radioData" :key="item.id" :class="{'active':item.name == focusLi ? true : false}" @click="navOne(item)">{{item.name}}<span v-if="item.isNew">新</span></li>
        </ul>
        <ul class="navTwo" :class="{'active':isActive}">
          <li v-for="(item,index) in navTwoCon" :key="index" :class="{'active1':item.name == focusLi2 ? true : false}" @click="navTwo(item)">{{item.name}}</li>
        </ul>
        </div>
      </div>
